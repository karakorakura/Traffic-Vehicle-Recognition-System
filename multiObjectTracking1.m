...
% foreground objects, and displaying results.
    ...
    function obj = setupSystemObjects()
        ...
        obj.reader = vision.VideoFileReader('C:\Users\shivam arora\Videos\Youtube Videos\ISI sTUDY\ExtraWork\Chandni_Chowk_X-ing_4_E.avi');
        ...
        %   'NumTrainingFrames', 8000, 'MinimumBackgroundRatio', 0.7);
        %%
    buf = 400;
    stride = 15;
    ...
    %%
        ...
        obj.blobAnalyser = vision.BlobAnalysis('BoundingBoxOutputPort', true, ...
            'AreaOutputPort', true, 'CentroidOutputPort', true, ...
            'MinimumBlobArea', 400,'MaximumBlobArea', 640*360/6);
...
% remove noisy pixels and to fill the holes in the remaining blobs.  
    ...
    function [centroids, bboxes, mask] = detectObjects(frame)
        ...
        foreground = (abs(rgb2gray(background)-rgb2gray(frame))>0.07);
        se = strel('square', 3);
        filteredForeground = imerode(foreground, se);
        %filteredForeground = imopen(foreground, se);
        filteredForeground = imopen(foreground, se);
        mask = filteredForeground ;
